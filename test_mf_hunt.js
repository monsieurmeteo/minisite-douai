...
async function huntEndpoint() {
    const bases = [
        'https://public-api.meteofrance.fr/public/DPPaquetObs/v1',
        'https://public-api.meteofrance.fr/public/DPPaquetObs'
    ];
    const paths = [
        '/paquet/stations/infrahoraire-6m',
        '/stations/infrahoraire-6m',
        '/infrahoraire-6m',
        '/paquet/station-infra-horaire-6m'
    ];

    for (const base of bases) {
        for (const path of paths) {
            const url = `${base}${path}?date=2026-01-18T18:00:00Z&format=json`;
            console.log(`Checking: ${url}`);
            try {
                const res = await fetch(url, { headers: { 'apikey': token } });
                console.log(` -> Status: ${res.status}`);
                if (res.status === 200) { console.log("FOUND IT!"); return; }
            } catch (e) { }
        }
    }
}
