...
async function testFinalVerify() {
    const stationId = '59178001';
    const urlObs = `https://public-api.meteofrance.fr/public/DPObs/v1/station/horaire?id_station=${stationId}&format=json`;

    console.log("Verifying token with DPObs (apikey header)...");
    try {
        const res = await fetch(urlObs, { headers: { 'accept': 'application/json', 'apikey': token } });
