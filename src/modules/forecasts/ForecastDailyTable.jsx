...
// Generic component to show a slice of daily forecast
export default function ForecastDailyTable({ title, startDay = 0, endDay = 5 }) {
    ...
    useEffect(() => {
        async function loadData() {
            ...
            // FILTER based on start/end day
            // Forecast comes as J, J+1, ...
            const sliced = forecast.slice(startDay, endDay);
            setData(sliced);
            setLoading(false);
        }
        loadData();
    }, [startDay, endDay, location]);

    if (loading) return <div className="card p-4">Chargement des prévisions...</div>;

    // Check if we have data for the requested range
    if (data.length === 0) return <div className="card p-4">Données indisponibles pour cette période.</div>;
