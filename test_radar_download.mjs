...
const API_URL = 'https://public-api.meteofrance.fr/public/DPPaquetRadar/v1/mosaique/paquet';

async function downloadRadar() {
    console.log("--- DOWNLOAD RADAR PACKET ---");
    try {
        ...
        if (response.ok) {
            ...
            fs.writeFileSync('test_radar.data', Buffer.from(buffer));
            console.log("File saved as test_radar.data");
        }
    } catch (error) {
